...
1. **`template.yaml`** — the creation wizard
   ...
   - **Monorepo component templates**: add a field to auto-read the parent system's repo name via `EntitySelectionPicker` with `ui:property: metadata.annotations["witboost.com/repo-name"]` (the annotation key is provider-agnostic). Build `repoUrl` from it in the publish step. Set `targetPath` to a subdirectory like `./components/${{ parameters.name | lower }}`, anchored with `&rootDirectory`, and register with `catalogInfoUrl`.
...
Use validators and validation commands already available in the repository or workspace. If no
validator exists, perform only targeted static checks relevant to the files and behavior changed.
Do not create temporary validation scripts unless the user explicitly requests one or a reusable
validator is clearly needed for the repository.
...
- **Prefer existing validators over ad hoc validation scripts.** Use repository or workspace validators when available. If none exists, do targeted static checks for the changed surface; never create a temporary validation script unless the user explicitly requests one or it will be kept as a reusable repository tool.
