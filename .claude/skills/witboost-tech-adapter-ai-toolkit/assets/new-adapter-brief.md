...
- Shared Witboost workspace assumed, or `--standalone` used with `.witboost/` copied and setup CLI run for harness(es):
...
(and, with `--standalone`, harness files). The repository is untouched by
construction unless the user made their own changes before opening this
...
the scaffold itself. Dedicated agents are already available either way
(from the shared workspace, or locally if `--standalone` was used): point
the user at "Witboost Adapter High-Level Design" (recommended first,
skippable) or "Witboost Adapter New Feature" directly instead of
...
`SKIP_GIT_STATE`, matching whatever mode bootstrap used) to confirm whether
it has diverged, then continue with
